...
module.exports = (app) => {
  ...
  app.get("/api/notes", (req, res) => {
    const allNotes = fs.readFileSync("./db/db.json", "utf8");
    res.send(allNotes);
  });
  ...
  app.post("/api/notes", (req, res) => {
    ...
    fs.writeFile(
      ...
      (err) => {
        err ? console.error(err) : console.log("Successfully wrote db.json");
      }
    );
    res.json(newNote);
  });
  ...
  app.delete("/api/notes/:id", (req, res) => {
    ...
    let notesArr = JSON.parse(
      fs.readFileSync(path.join("../db/db.json"), "utf8")
    );
